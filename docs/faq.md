...
- Apply inserts or merges into the existing 1.x library; it does not wipe it.
- Apply commits as one SQLite transaction; individual bookmark failures are
  soft-skipped inside that transaction, while a crash before commit rolls the
  whole apply back.
...
- Soft-skipped bookmark failures still yield HTTP 207 / CLI exit 1 after the
  successful part of the apply is committed.
...
If the database has multiple users, `--owner` is required. You can optionally
pass `--password` / `--password-file` to verify the v0.5 account password
before import. Supported archives include `.zip`, `.tar`, `.tar.gz`/`.tgz`,
`.tar.bz2`, and `.tar.xz`. Grimoire 1.x does not recreate multi-user logins —
it imports one owner's library into the local single-user library. PocketBase-
era backups are not supported. See the [migration API reference](https://github.com/goniszewski/grimoire/blob/main/API.md#migrate).
