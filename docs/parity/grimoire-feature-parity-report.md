# Grimoire vs Legacy Grimoire Feature Parity Report

Date: 2026-06-02

## Scope

This report compares the current Grimoire workspace against `goniszewski/grimoire` (legacy v0.x) for practical feature parity.

- Grimoire baseline: `3b73011715e74296f0aa4f91ede120409441fb6c` (`2026-05-29`, `chore: add UI audit and task reporting workflow`)
- Legacy Grimoire baseline: `7bb6e74c1f6c026040c5be53d5d45eac7c44ee91` (`2025-05-31`, `added one-click deploy via deploystack (#188)`)
- Grimoire source inspected from this local workspace.
- Legacy Grimoire source inspected from a fresh GitHub clone.
...
legacy Grimoire server parity. Deferred and rejected rows in
...
Grimoire is not at full legacy legacy Grimoire server parity, but the current workspace has
closed nearly all approved local-first parity gaps from the current batch.
Grimoire now has bookmark notes, read state, archive/trash flows, category
...
integration clients. Legacy Grimoire's multi-user/account model, profile/admin
...
This batch keeps Grimoire's product model intact:
...
- **Local integrations only:** MCP, CLI, protected local capture, local
  ...
  browser-extension/bookmarklet clients and legacy Grimoire-compatible endpoint aliases
...
| --- | --- | --- | --- |
| Multi-user accounts, signup, login, sessions, profile management, admin roles, and per-user backup scoping | PAR-028 through PAR-034 | These features would change Grimoire from a local personal app into a server/account product and would affect storage, backups, diagnostics, MCP, settings, auth, and support. | Reopen only through a separate multi-user/server product decision with schema, migration, auth/session, backup-scoping, and threat-model work approved together. |
...
| legacy Grimoire-compatible endpoint aliases or adapter routes | PAR-025 | Aliases would add long-term API compatibility surface without a current local-first migration or extension requirement. | Reconsider only if a concrete integration or migration path proves aliases materially reduce user migration cost without weakening the canonical Grimoire API contract. |
...
| Sessions and API auth | Lucia sessions plus bearer-token API patterns | First-party REST remains loopback-trusted; MCP and protected capture use managed bearer tokens | Partial local integration parity | legacy Grimoire-style sessions are out of scope. TASK-102 adds scoped local integration-token auth without turning Grimoire into a multi-user server. |
...
| Bookmark CRUD | Add, edit, delete, list, detail | Add, list, detail, soft delete, restore, permanent delete, update selected fields | Partial by deferred scope | Grimoire updates title, category, tags, pin, archive, read state, read-later state, open metrics, and notes. URL and extracted summary mutation remain deferred, and unsupported edit affordances were removed by TASK-094. |
...
| Metadata and extraction | Metadata fetch plus article extraction | Async fetch/extract/AI/embed/index pipeline with site-specific extractors | Grimoire ahead | Grimoire has stronger background processing and recovery behavior. |
...
| Flagged/starred | Flagged bookmark state | Pinning exists and maps to legacy Grimoire starred/favorite; read-later is a separate flag | Intentional mapping | PAR-007 maps legacy Grimoire starred/favorite state to existing Grimoire pinning. TASK-089 adds separate read-later persistence and UI controls. |
...
| Search | Fuzzy search with filters | Keyword FTS plus optional semantic/hybrid search, server-side filtering, sorting, pagination, and aggregate counts | Grimoire ahead | TASK-114 through TASK-119 completed pagination, server-driven sorting, approved parity filters, persisted view preferences, aggregate counts, and large-library checks. |
...
| Export | Export/migration capabilities | JSON/CSV export with filters plus notes, read state, archive state, pinned/starred mapping, read-later state, and opened metrics | At parity | TASK-111 completed approved export parity fields for active-library JSON/CSV exports. Direct legacy Grimoire backup import remains deferred separately. |
| Migration | PocketBase/Grimoire migration helpers | None | Deferred non-goal for direct legacy Grimoire backup import | TASK-112 documents the source data shape and Grimoire field mapping in [grimoire-backup-import-shape.md](./grimoire-backup-import-shape.md). Runtime direct import remains future scope. |
...
| Deployment | Docker/self-hosting-oriented docs | Native installer, Homebrew, local daemon, Docker guidance, update docs | Intentional product difference | Grimoire is stronger for local install. Public-server and multi-user deployment modes are out of scope for this parity batch. |
| Backups and restore | Not a primary parity strength | Local/S3 backups, encrypted packages, restore recovery | Grimoire ahead | This is beyond Grimoire parity. |
| Timeline and suggestions | Limited compared with Grimoire | Timeline events, review queue, AI organization suggestions | Grimoire ahead | Local-first AI organization is a Grimoire differentiator. |
| App lock and preferences | Not a core Grimoire feature | Local app lock, theme, view, button-label preferences | Grimoire ahead | App lock does not replace server authentication. |
...
2. Treat URL/summary bookmark mutation, direct legacy Grimoire backup import tooling,
...
`docs/release-checklist.md` now includes a recurring legacy Grimoire parity review for
