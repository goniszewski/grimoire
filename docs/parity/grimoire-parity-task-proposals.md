...
| PAR-041 | Add Grimoire backup import tooling only after browser/Netscape import and JSON/CSV export parity are stable. | Low | Deferred | Handoff documented | Runtime import tooling remains deferred. Current batch focuses on robust browser/Netscape import and export parity. Future implementation should start from `docs/parity/grimoire-backup-import-shape.md`; deferral record: `TASK-112`. |
