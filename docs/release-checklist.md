...
|---|---|---|
| API contract and docs | TASK-102, TASK-103, TASK-104, TASK-106, PAR-021 through PAR-026 | Run `npm run docs:api:check` for API docs drift and `npm run docs:openapi:check` for the OpenAPI-only path; run focused daemon tests for changed routes, auth, origin, or contract helpers; confirm `API.md`, `docs/api-contract.json`, and `docs/openapi.json` match the source contract. |
