...
| Grimoire parity acceptance | TASK-134 added [parity-acceptance-checklist.md](./parity/parity-acceptance-checklist.md), which maps every approved done parity row to daemon, frontend, generated API, visual, e2e, performance, or documentation verification evidence. Deferred multi-user/server mode, URL/summary mutation, direct Grimoire backup import, packaged browser clients, endpoint aliases, and bookmark importance remain intentional non-goals or future product decisions. | Passed for approved local-first parity scope |
