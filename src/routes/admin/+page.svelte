<script lang="ts">
	...
	const tabs = ['Users', 'AI Integrations', 'PocketBase Settings [read-only]'];
...
{#if !$user.isAdmin && !$user.model}
	<p>Not logged in.</p>
{:else if !$user.isAdmin && $user.model}
	<p>Not admin.</p>
{:else}
		...
		<div class="flex flex-col my-4 gap-2">
			...
			{:else}
						...
						<div class="collapse-content">
							<div class="overflow-x-auto">
								<form>
									...
									{#if $updatedSettings.llm.provider === 'openai' || ($updatedSettings.llm.provider === '' && data.adminData.settings.llm.provider === 'openai')}
										<div class="form-control">
									...
									{#if $updatedSettings.llm.provider === 'ollama' || ($updatedSettings.llm.provider === '' && data.adminData.settings.llm.provider === 'ollama')}
										<div class="form-control">
				...
				{:else if $activeTab === 'PocketBase Settings [read-only]'}
					...
					<!-- Authentication -->
				{/if}
				<button class="btn btn-primary w-[8rem] ml-auto" on:click={() => {}}>Save</button>
			{/if}
		</div>
	</div>
{/if}
